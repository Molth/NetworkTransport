...
namespace kcp
{
    ...
    public static unsafe partial class KCP
    {
        ...
        public const uint IKCP_CMD_PUSH_NEXT = 85; // cmd: push data next
        public const uint IKCP_CMD_PUSH_ZERO_FRG = 86; // cmd: push data 0 frg
        public const uint IKCP_CMD_PUSH_ZERO_FRG_NEXT = 87; // cmd: push data 0 frg next
        ...
        public const uint IKCP_OVERHEAD = 10;
        ...
        public const uint IKCP_RESERVED_OVERHEAD = 10;
        public const uint IKCP_FRG_LIMIT = 255;
        ...
        // check log mask
        public static int ikcp_canlog(IKCPCB* kcp, int mask)
        {
        ...
        public static int ikcp_output(IKCPCB* kcp, void* data, int size)
        {
            ...
            if (ikcp_canlog(kcp, (int)IKCP_LOG_OUTPUT) != 0)
            {
                ikcp_log(kcp, (int)IKCP_LOG_OUTPUT, "[RO] {0} bytes", (long)size);
            }
        ...
        // create a new kcpcb
        //---------------------------------------------------------------------
        public static IKCPCB* ikcp_create(IKCPCB* kcp, void* user)
        {
            ...
            kcp->mss = kcp->mtu - IKCP_RESERVED_OVERHEAD - IKCP_OVERHEAD;
            kcp->stream = 0;
        ...
        public static int ikcp_send(IKCPCB* kcp, byte* buffer, int len)
        {
            ...
            if (count > IKCP_FRG_LIMIT || count >= (int)kcp->rcv_wnd)
            {
        ...
        {
            uint una = kcp->rcv_nxt;

            if (_itimediff(sn, una) <= 0)
                return;
        ...
        // parse data
        //---------------------------------------------------------------------
        public static void ikcp_parse_data(IKCPCB* kcp, byte* data, byte frg, uint sn, uint len)
        {
            IQUEUEHEAD* p, prev;
            int repeat = 0;
            ...
            if (repeat == 0)
            {
                ...
                newseg = ikcp_segment_new(kcp, (int)len);
                newseg->frg = frg;
                newseg->sn = sn;
                newseg->len = len;
        ...
        public static int ikcp_input(IKCPCB* kcp, byte* data, long size)
        {
            ...
            if (ikcp_canlog(kcp, (int)IKCP_LOG_INPUT) != 0)
            {
                ikcp_log(kcp, (int)IKCP_LOG_INPUT, "[RI] {0} bytes", (int)size);
            }

            if (data == null || (int)size < (int)IKCP_RESERVED_OVERHEAD) return -1;
            ...
            size -= (int)IKCP_RESERVED_OVERHEAD;
            ...
            while (true)
            {
                ...
                switch (cmd)
                {
                    case (byte)IKCP_CMD_ACK:
                        if ((long)size < (long)6) return -2;
                        ...
                        break;
                    case (byte)IKCP_CMD_PUSH:
                        if ((long)size < (long)9) return -2;
                        ...
                        if (_itimediff(sn, kcp->rcv_nxt + kcp->rcv_wnd) < 0)
                        {
                            ...
                            {
                                ikcp_parse_data(kcp, data, frg, sn, len);
                            }
                        ...
                        size -= len;
                        break;
                    case (byte)IKCP_CMD_PUSH_NEXT:
                        if (pushed == 0 || (long)size < (long)5) return -2;
                        ...
                        size -= len;
                        break;
                    case (byte)IKCP_CMD_PUSH_ZERO_FRG:
                        if ((long)size < (long)8) return -2;
                        ...
                        size -= len;
                        break;
                    case (byte)IKCP_CMD_PUSH_ZERO_FRG_NEXT:
                        if (pushed == 0 || (long)size < (long)4) return -2;
                        ...
                        size -= len;
                        break;
                    case (byte)IKCP_CMD_WASK:
                        // ready to send back IKCP_CMD_WINS in ikcp_flush
                        // tell remote my window size
                        kcp->probe |= IKCP_ASK_TELL;
                        if (ikcp_canlog(kcp, (int)IKCP_LOG_IN_PROBE) != 0)
                        {
                            ikcp_log(kcp, (int)IKCP_LOG_IN_PROBE, "input probe");
                        }

                        break;
                    case (byte)IKCP_CMD_WINS:
                        // do nothing
        ...
        // ikcp_encode_seg
        //---------------------------------------------------------------------
        public static byte* ikcp_encode_seg(byte* ptr, IKCPSEG* seg, int* encoded, uint* sn)
        {
            if (seg->frg == 0)
            {
                if (*encoded == 0 || *sn + 1 != seg->sn)
                {
                    ptr = ikcp_encode8u(ptr, (byte)IKCP_CMD_PUSH_ZERO_FRG);
                    ptr = ikcp_encode32u(ptr, seg->sn);
                }
                else
                {
                    ptr = ikcp_encode8u(ptr, (byte)IKCP_CMD_PUSH_ZERO_FRG_NEXT);
                }
            }
            else
            {
                if (*encoded == 0 || *sn + 1 != seg->sn)
                {
                    ptr = ikcp_encode8u(ptr, (byte)IKCP_CMD_PUSH);
                    ptr = ikcp_encode8u(ptr, (byte)seg->frg);
                    ptr = ikcp_encode32u(ptr, seg->sn);
                }
                else
                {
                    ptr = ikcp_encode8u(ptr, (byte)IKCP_CMD_PUSH_NEXT);
                    ptr = ikcp_encode8u(ptr, (byte)seg->frg);
                }
            }
        ...
        // ikcp_flush
        //---------------------------------------------------------------------
        public static void ikcp_flush(IKCPCB* kcp, byte* buffer)
        {
            ...
            IQUEUEHEAD* p;
            int change = 0;
            int lost = 0, encoded = 0;
            uint sn, ts;

            // 'ikcp_update' haven't been called. 
            if (kcp->updated == 0) return;

            uint wnd = (uint)ikcp_wnd_unused(kcp);
            uint una = kcp->rcv_nxt;

            buffer = ikcp_encode32u(buffer, current);
            buffer = ikcp_encode16u(buffer, (ushort)wnd);
            ...
            for (i = 0; i < count; i++)
            {
                ...
                if (_itimediff(sn, una) > 0)
                {
                    size = (int)(ptr - buffer) + (int)IKCP_RESERVED_OVERHEAD;
                    if (size + (int)9 > (int)kcp->mtu)
                    {
                        ikcp_output(kcp, buffer - (int)IKCP_RESERVED_OVERHEAD, size);
                        ptr = buffer;
                    }

                    ptr = ikcp_encode8u(ptr, (byte)IKCP_CMD_ACK);
            ...
            {
                size = (int)(ptr - buffer) + (int)IKCP_RESERVED_OVERHEAD;
                if (size + (int)1 > (int)kcp->mtu)
                {
                    ikcp_output(kcp, buffer - (int)IKCP_RESERVED_OVERHEAD, size);
                    ptr = buffer;
                }

                ptr = ikcp_encode8u(ptr, (byte)IKCP_CMD_WASK);
            }
            ...
            if ((kcp->probe & IKCP_ASK_TELL) != 0)
            {
                ...
                ptr = ikcp_encode8u(ptr, (byte)IKCP_CMD_WINS);
            }
            ...
            for (p = kcp->snd_buf.next; p != &kcp->snd_buf; p = p->next)
            {
                ...
                if (needsend != 0)
                {
                    ...
                    ptr = ikcp_encode_seg(ptr, segment, &encoded, &sn);
            ...
            size = (int)(ptr - buffer) + (int)IKCP_RESERVED_OVERHEAD;
            if (size > (int)IKCP_RESERVED_OVERHEAD || kcp->inputed != 0)
            {
                ikcp_output(kcp, buffer - (int)IKCP_RESERVED_OVERHEAD, size);
            }
        ...
        public static void ikcp_update(IKCPCB* kcp, uint current, byte* buffer)
        {
            int slap;
        ...
        {
            if (mtu < 50 || mtu < (int)IKCP_RESERVED_OVERHEAD + (int)IKCP_OVERHEAD)
                return -1;
            kcp->mtu = (uint)mtu;
            kcp->mss = kcp->mtu - (int)IKCP_RESERVED_OVERHEAD - IKCP_OVERHEAD;
            return 0;
        }
